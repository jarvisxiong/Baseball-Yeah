...
public class ReportController extends BaseController {
    ...
    @RequestMapping(value = "/gotoVoucherReport")
    public ModelAndView gotoVoucherReport(HttpServletRequest request) {
        if (request.getSession().getAttribute("user") != null) {
            return new ModelAndView("report/voucherReport/voucherReport");
        } else {
            return new ModelAndView("/");
        }
    }

	@RequestMapping(value="/gotoBusinessReport")
	public ModelAndView gotoBusinessReport(HttpServletRequest request){
		if (request.getSession().getAttribute("user") != null) {
			return new ModelAndView("report/businesReport/businesReport");
		}else {
			return new ModelAndView("/");
		}
	}
