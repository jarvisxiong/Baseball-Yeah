...
import com.rofour.baseball.dao.order.mapper.OrderVerifyMapper;
...
public class OrderVerifyServiceImpl implements OrderVerifyService {
    ...
    public List<UserCheckInfo> getAuditUsers(UserCheckInfo user) throws Exception {
        ...
        String startTime = userCheckBean.getStartDate().toString();
        String endTime = userCheckBean.getEndDate().toString();
        if (StringUtils.isNotBlank(startTime)) {
            userCheckBean.setStartDate(startTime + " 00:00:00");
        }
        if (StringUtils.isNotBlank(endTime)) {
            userCheckBean.setEndDate(endTime + " 23:59:59");
        }
    ...
    public int getAuditUsersCount(UserCheckInfo user) throws Exception {
        UserCheckBean userCheckBean = new UserCheckBean();
        AxpUtils.copyProperties(user, userCheckBean);
        String startTime = user.getStartDate().toString();
        String endTime = user.getEndDate().toString();
        if (StringUtils.isNotBlank(startTime)) {
            user.setStartDate(startTime + " 00:00:00");
        }
        if (StringUtils.isNotBlank(endTime)) {
            user.setEndDate(endTime + " 23:59:59");
        }
     ...
     */
    @Override
    public void setLabel(long pUserId, String remark) throws Exception {
        Map map = new HashMap();
        map.put("pUserId", pUserId);
        map.put("remark", remark);
        orderVerifyMapper.setLabel(map);
    }
